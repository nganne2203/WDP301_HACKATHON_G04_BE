...
const listTimelines = async (req, res, next) => {
  try {
    const { timelines, pagination } = await TIMELINE_SERVICE.listTimelines(
...
const createTimeline = async (req, res, next) => {
  try {
    const timeline = await TIMELINE_SERVICE.createTimeline(
...
const updateTimeline = async (req, res, next) => {
  try {
    const timeline = await TIMELINE_SERVICE.updateTimeline(
      req.params.id,
      req.validated?.body || req.body
    )
